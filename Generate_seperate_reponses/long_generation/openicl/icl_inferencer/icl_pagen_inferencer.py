...
class PAGenInferencer(BaseInferencer):
    ...
    def inference(self, retriever: BaseRetriever, ice_template: Optional[PromptTemplate] = None,
        ...
        for entry in tqdm(dataloader, disable=not self.is_main_process):
            ...
            if not self.call_api:
                with torch.no_grad():
                        ...
                        if self.args.data_name == "ocr":
                            while True:
                                ...
                                if prompt_len < 2000:
                                    ...
                                    second_comma_index = en.find(",", first_comma_index+1)
                    ...
                    if force_words is not None:
                        ...
                        outputs = self.model.generate(input_ids=tokenized_data.input_ids,
                                                      attention_mask=tokenized_data.attention_mask,
                                                      eos_token_id=[self.tokenizer.eos_token_id, 434],
                                                      pad_token_id=self.tokenizer.pad_token_id,
                                                      **self.generation_kwargs)
            ...
            for prediction, output in zip(generated, complete_output):
                ...
                output_handler.write_now(output_json_filepath, output_json_filename)
                index = index + 1
    ...
    def embedding(self, retriever: BaseRetriever, ice_template: Optional[PromptTemplate] = None,
        ...
        for i in range(len(predictions)):
            ...
            output.append([predictions[str(i)]['output'][predictions[str(i)]['output'].rfind("Dialogue:"):]])
            origin_prompt.append([predictions[str(i)]['origin_prompt'][predictions[str(i)]['origin_prompt'].rfind("Dialogue:"):]])
        ...
        df.to_csv('embedding/'+output_json_filename+'embedding.csv', index=False)
