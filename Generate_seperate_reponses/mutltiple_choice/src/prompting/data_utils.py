...
class BaseProcessor:
    @cached_property
    def dataset(self):
        return load_dataset(self.dataset_name, cache_dir='./data')
    ...
    @cached_property
    def test_split(self):
        if self.dataset_name == "SetFit/sst2":
            return self.dataset["validation"]
        else:
    ...
    def generate_datasets(self, seed: int, mode: str):
        ...
        labeled_train_indices = shuffled[0:8000]
        labeled_val_indices = shuffled[0:1]
        ...
        print("train_dataset", len(self.train_dataset), "#"*100)
        ...
        print("test_size", test_size, "#"*100)

        # always use the same test set 42
    ...
    def create_prompts_diff(
        ...
        for train_indice in train_indices:
            training_prompt = self.get_training_prompt(
                train_indice, train_split=train_split
            )
        ...
        for index, eval_example in enumerate(dataset):
            prompt = Prompt(training_prompt_list[index] + self.fill_eval_template(eval_example))
            prompts.append(prompt)
        #print("self.calibration_examples", self.calibration_examples)

        for calibrate_example in self.calibration_examples:
            calibrate_prompt = Prompt(
                training_prompt_list[index] + self.fill_calibration_template(calibrate_example),
                is_calibration_prompt=True,
            )
            cali_prompts.append(calibrate_prompt)
...
class AGNewsProcessor(BaseProcessor):
    def __init__(self, seed: int, mode: str):
        ...
        self.val_split = None
        self.prompt_start = (
            "Classify the news based on whether their"
            "type is a World, Sports, Business"
            ", or Technology.\n\n"
        )
